...
class magpie_shader:
    ...
    def init(self, w, h):
        ...
        for i in range(self.pass_max):
            for s in self.parsed_IN[i]:
                if s == 'INPUT': self.SRV.append(self.INPUT)
                else: self.SRV.append(self.T[int(s.replace("T", ""))])

        self.UAV = []
        for i in range(self.pass_max):
            for s in self.parsed_OUT[i]:
                if s == 'OUTPUT': self.UAV.append(self.OUTPUT)
                else: self.UAV.append(self.T[int(s.replace("T", ""))])
    ...
    def compute(self):
        for i in range(self.pass_max):
            ...
            ####### possible move this to init section?
