...
# TODO test_all_shaders: load multiple shaders and click to loop title/display within single window, for comparison
if __name__ == "__main__":
    ...
    #s1 = magpie_shader("CuNNy-faster-NVL.hlsl")
    #s1 = magpie_shader("CuNNy-fast-NVL.hlsl")
    ...
    s1.upload(img_data)
    s1.compute()
    print("Close all terminal windows first for accurate results. REF 18 20 26 ms")
    ...
    if(DISPLAY):
        ...
        def on_draw():
            s1.download()
            img = pyglet.image.ImageData(s1.OUTPUT.row_pitch//4, s1.OUTPUT.height, "RGBA", s1.readback_buffer.readback(), pitch=-s1.OUTPUT.row_pitch)
            img.blit(0, 0)
