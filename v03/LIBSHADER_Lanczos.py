...
with open("Lanczos.hlsl", 'r') as fp: shader = fp.read()
s1 = hlsl.compile(shader)

def init_buffer(width, height, output_width=0, output_height=0):
...
def init_and_compute(buffer, width, height, output_width=0, output_height=0): # for one off run
    global staging_buffer, readback_buffer
    global INPUT, OUTPUT, CB0
    if output_width == 0: output_width = width*2
    if output_height == 0: output_height = height*2
