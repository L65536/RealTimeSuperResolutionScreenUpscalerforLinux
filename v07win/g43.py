...
def compute_worker():
    ...
    global count, total_time_compute, transparent
    global w, swapchain
    while running:
        # Reset out-of-bound mouse position, warp to the other side
        left, top, right, bottom = win32gui.GetWindowRect(hwnd)
        x, y = win32gui.GetCursorPos()        
        if x<left: win32api.SetCursorPos((right, y))
        if x>right: win32api.SetCursorPos((left, y))
        if y<top: win32api.SetCursorPos((x, bottom))
        if y>bottom: win32api.SetCursorPos((x, top))
...
while True:
    ...
    handle = None # capture handle

    keyboard.add_hotkey('tab', hotkey)
    while hwnd is None: time.sleep(1)

    _, _, clientW, clientH = win32gui.GetClientRect(hwnd)
    handle = GraphicsCaptureItem.try_create_from_window_id(WindowId(hwnd)) # convert window handle to GraphicsCaptureItem handle
    ...
    win32gui.PumpMessages() # Message Loop blocking
