...
class WmBase(wayland.xdg_wm_base):
    def on_ping(self, serial): self.pong(serial) # Respond to compositor pings.
...
class Registry(wayland.wl_registry):
    ...
    def on_global(self, name, interface, version):
        ...
        if hasattr(self, interface): setattr(self, interface, self.bind(name, interface, version))

@wayland_class("wl_display")
class Display(wayland.wl_display):
    def on_error(self, object_id, code, message): # Handle fatal errors
...
class MainWindow:
    ...
    def on_configure(self, width, height, states): # Handle window resize events.
        if width and height:
    ...
    def redraw(self):
        ...
        # Capture using NVFBC pipewire backend
        t = time.perf_counter()
        addr = cap.capture(1) # ctypes.c_void_p
        ...
        #bitmap = numpy.frombuffer(buf, dtype=numpy.uint32) # keep this section for compute
        #print(clientW, clientH)

        # Copy image to buffer
        pixels = ctypes.cast(ptr, ctypes.POINTER(ctypes.c_uint32))
        ctypes.memmove(pixels, addr, clientW * clientH * 4)
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.display.dispatch_timeout(1)  # 30 FPS
            #self.redraw()
