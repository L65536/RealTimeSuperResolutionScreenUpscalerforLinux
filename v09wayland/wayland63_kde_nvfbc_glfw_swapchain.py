...
# Virtual monitor's position and layout can be checked/changed at KDE's monitor configuration page.
...
# Use [Wheel click] to minimize or [Window] key to access taskbar.
...
def custom_cursor():
    ...
    text = "Move cursor beyond right side boundary to access virtual screen >>>"
    draw.text((128,0), text, fill=(255, 255, 255), font=font)
    text = "[Wheel click] here to minimize; [Window] key to access taskbar."
    draw.text((128,64), text, fill=(255, 255, 255), font=font)

    return glfw.create_cursor(image, 30, 0) # hotspot at top-left corner
...
window = glfw.create_window(width, height, "SR Magnifier", monitor, None)
# window = glfw.create_window(width, height, "SR Magnifier", None, None)
...
swapchain = compushady.Swapchain((glfw.get_wayland_display(), glfw.get_wayland_window(window)),
            compushady.formats.B8G8R8A8_UNORM, 3, None, width, height)

print('\nThe magnified window, "SR Magnifier", should be available minimized on the taskbar.')
print('Use [Middle click] to minimize or [Window] key to access taskbar.')
...
print(f'Average capture time/frame = {(total_time_capture/count)*1000:.2f} (ms)') # 17-22 ms for 60hz (including waiting for screen updates)
print(f'Average compute time/frame = {(total_time_compute/count)*1000:.2f} (ms)') # 6-7 ms for 1080p source
print(f'Average display time/frame = {(total_time_display/count)*1000:.2f} (ms)') # 2 ms for 4k output
print('End')
